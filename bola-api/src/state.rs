...
use mangle_api_core::{auth::{auth_pages::{AuthPages}, openid::{google::{GoogleOIDC}, OIDCState}}, neo_api::NeoApiConfig};

use crate::{db::DB, leaderboard::Leaderboard, LoginTokenGranter, multiplayer::Multiplayer, tournament::Tournament, ws_api::WsApiHandler};
...
macro_rules! new_global {
    ($config: expr, $https_identity: expr, $aws_config: expr) => {{
        ...
        let auth_pages = manglext::immut_leak(mangle_api_core::auth::auth_pages::AuthPages::new(mangle_api_core::auth::auth_pages::AuthPagesSrc {
        ...
        let node = manglext::immut_leak(mangle_api_core::distributed::Node::new(
        ...
        .await?);
        let db = manglext::immut_leak($crate::db::DB::new(&$aws_config, $config.bola_profiles_table));

        let goidc = manglext::immut_leak(mangle_api_core::auth::openid::google::new_google_oidc_from_file(
        ...
        let leaderboard = manglext::immut_leak($crate::leaderboard::Leaderboard::new(db.clone(), node, 5).await?);
        let login_tokens = manglext::immut_leak(LoginTokenGranter::new($config.token_duration));
        let ws_api = manglext::immut_leak(mangle_api_core::neo_api::NeoApiConfig::new(WS_PING_DELAY, $crate::ws_api::WsApiHandler::new(leaderboard, db, &goidc.0, login_tokens)));
        ...
        $crate::state::GlobalState {
            ...
            tournament: manglext::immut_leak($crate::tournament::Tournament::new($config.start_week_time)),
